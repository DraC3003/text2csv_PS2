...
class PatientForm:
    ...
    def setup_ui(self):
        ...
        fields = [
            ...
            ('date_of_birth', 'Date of Birth (YYYY-MM-DD) - Optional but recommended for accurate ranges', 'entry'),
        ...
        for i, (field_name, label_text, field_type) in enumerate(fields):
            ...
            if field_type == 'entry':
                ...
                combo['values'] = ['Male', 'Female', 'Other', 'Prefer not to say']
    ...
    def validate_form(self, update=False):
        ...
        dob = self.form_vars['date_of_birth'].get().strip()
        if dob:
            try:
    ...
    def populate_form(self, patient_info):
        ...
        field_mapping = {
            ...
            'date_of_birth': patient_info[3] or '',
    ...
    def refresh_results_display(self):
        ...
        for patient in patients:  # Show ALL patients (removed limit)
            ...
            for result in results:  # Show ALL results per patient (removed limit)
                ...
                # Determine status with age/gender adjustments
                status = "Normal"
                if normal_min is not None and normal_max is not None:
                    from report_generator import ReportGenerator
                    rg = ReportGenerator(self.db_manager)
                    status_code = rg.determine_test_status(test_value, normal_min, normal_max, test_name, age, gender)
    ...
    def generate_template(self):
        ...
        if file_path:
            ...
            if success:
                ...
                self.preview_text.insert(tk.END, "- first_name, last_name (optional)\n")
                self.preview_text.insert(tk.END, "- date_of_birth (YYYY-MM-DD format for age calculation)\n")
                self.preview_text.insert(tk.END, "- gender (Male/Female/Other for adjusted normal ranges)\n")
                self.preview_text.insert(tk.END, "- test_name (required)\n")
                self.preview_text.insert(tk.END, "- test_value (required)\n")
                self.preview_text.insert(tk.END, "- test_date (required, YYYY-MM-DD format)\n")
                self.preview_text.insert(tk.END, "- unit, lab_technician, notes (optional)\n\n")
                self.preview_text.insert(tk.END, "Note: Age and gender are used to apply appropriate\n")
                self.preview_text.insert(tk.END, "normal ranges for different demographic groups.")
            else:
