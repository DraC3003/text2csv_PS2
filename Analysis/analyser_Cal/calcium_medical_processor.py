"""
Simple Calcium Medical Data Processor
Focus: Only Calcium analysis from EIPL CSV files
"""
...
# Calcium Clinical Ranges (Standard Medical Guidelines)
CALCIUM_CLINICAL_RANGES = {
    'units': 'mg/dL',
    'Total_Calcium': {
        'Normal': (8.5, 10.5),
        'Hypocalcemia_Mild': (7.0, 8.4),
        'Hypocalcemia_Severe': (0, 6.9),
        'Hypercalcemia_Mild': (10.6, 12.0),
        'Hypercalcemia_Severe': (12.1, 999)
    }
}
...
RISK_COLORS = {
    ...
    'Hypocalcemia_Mild': {'color': 'YELLOW', 'symbol': '[CAUTION]'},
    'Hypocalcemia_Severe': {'color': 'RED', 'symbol': '[URGENT]'},
    'Hypercalcemia_Mild': {'color': 'ORANGE', 'symbol': '[WARNING]'},
    'Hypercalcemia_Severe': {'color': 'PURPLE', 'symbol': '[CRITICAL]'}
}

# Excel Colors for actual cell backgrounds
EXCEL_COLORS = {
    'Normal': {'fill': '90EE90', 'font': '006400'},  # Light green background, dark green text
    'Hypocalcemia_Mild': {'fill': 'FFFF99', 'font': 'B8860B'},  # Light yellow background, dark goldenrod text
    'Hypocalcemia_Severe': {'fill': 'FFB6C1', 'font': '8B0000'},  # Light red background, dark red text
    'Hypercalcemia_Mild': {'fill': 'FFE4B5', 'font': 'FF8C00'},  # Light orange background, dark orange text
    'Hypercalcemia_Severe': {'fill': 'DDA0DD', 'font': '800080'}  # Light purple background, purple text
}

class CalciumProcessor:
    """Simple Calcium Data Processor for EIPL CSV files"""
    
    def __init__(self, append_mode=False):
        """Initialize the processor"""
        self.eipl_csv_file = self.find_eipl_csv()
        self.output_raw = "calcium_raw_data.csv"
        self.output_analyzed = "calcium_analyzed_results.csv"
        self.append_mode = append_mode
        
        logger.info(f"[INIT] Calcium Processor initialized (append_mode={append_mode})")
    
    def find_eipl_csv(self):
        """Find EIPL CSV file in current directory"""
        current_dir = Path(".")
        
        # Look for EIPL files with Cal in name
        for file in current_dir.glob("*.csv"):
            if file.name.upper().startswith("EIPL") and "CAL" in file.name.upper():
                logger.info(f"[DETECT] Found EIPL Calcium CSV: {file}")
                return str(file)
        
        # If no Cal-specific file, look for any EIPL file
        for file in current_dir.glob("*.csv"):
    ...
    def process_eipl_csv(self):
        """Process EIPL CSV file and extract calcium data"""
        ...
        try:
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    # Get calcium reading - try different column names
                    calcium_reading = None
                    
                    # Try BIO-CHEQ column (with or without space)
                    if 'BIO-CHEQ' in row and pd.notna(row['BIO-CHEQ']):
                        calcium_reading = row['BIO-CHEQ']
                    elif 'BIO-CHEQ ' in row and pd.notna(row['BIO-CHEQ ']):
                        calcium_reading = row['BIO-CHEQ ']
                    elif 'Device value (in conc.)' in row and pd.notna(row['Device value (in conc.)']):
                        calcium_reading = row['Device value (in conc.)']
                    
                    # Skip if no valid reading
                    if calcium_reading is None or pd.isna(calcium_reading) or str(calcium_reading).strip() == '':
                        continue
                    
                    # Clean up reading
                    calcium_reading_str = str(calcium_reading).strip()
                    
                    # Extract numeric value
                    numeric_match = re.search(r'(\d+\.?\d*)', calcium_reading_str)
                    if not numeric_match:
                        continue
                    
                    calcium_value = float(numeric_match.group(1))
                    
                    # Add units if not present
                    if not any(unit in calcium_reading_str.lower() for unit in ['mg/dl', 'mmol/l']):
                        calcium_reading_str = f"{calcium_value} mg/dL"
                    ...
                    record = {
                        'ID': patient_id,
                        'Test_name': 'Calcium',
                        'reading': calcium_reading_str,
                        'numeric_value': calcium_value,
                        'Age': age,
                        'Gender': gender,
            ...
            if not processed_records:
                if self.append_mode and skipped_count > 0:
                    ...
                    print(f"[INFO] No new calcium data found - {skipped_count} patients already in Excel file")
                else:
                    logger.warning("[WARNING] No valid calcium records found")
                return pd.DataFrame()
            
            result_df = pd.DataFrame(processed_records)
            if self.append_mode:
                logger.info(f"[APPEND] Found {len(result_df)} NEW calcium records (skipped {skipped_count} existing)")
                print(f"[APPEND] Found {len(result_df)} new patients (skipped {skipped_count} existing)")
            else:
                logger.info(f"[SUCCESS] Processed {len(result_df)} calcium records")
    ...
    def classify_calcium(self, calcium_value, age=None):
        """Classify calcium result based on clinical ranges"""
        
        ranges = CALCIUM_CLINICAL_RANGES['Total_Calcium']
        
        # Classification logic for total calcium
        if calcium_value < 7.0:
            return 'Hypocalcemia_Severe', ranges['Hypocalcemia_Severe']
        elif calcium_value < 8.5:
            return 'Hypocalcemia_Mild', ranges['Hypocalcemia_Mild']
        elif calcium_value <= 10.5:
            return 'Normal', ranges['Normal']
        elif calcium_value <= 12.0:
            return 'Hypercalcemia_Mild', ranges['Hypercalcemia_Mild']
        else:
            return 'Hypercalcemia_Severe', ranges['Hypercalcemia_Severe']
    
    def analyze_calcium_data(self, raw_data):
        """Analyze calcium data and create classifications"""
        if raw_data.empty:
            logger.warning("[WARNING] No data to analyze")
            return pd.DataFrame()
        
        logger.info(f"[ANALYZE] Analyzing {len(raw_data)} calcium records")
        
        analyzed_records = []
        
        for _, row in raw_data.iterrows():
            try:
                calcium_value = row['numeric_value']
                age = row['Age']
                gender = row['Gender']
                
                # Classify
                classification, classification_range = self.classify_calcium(calcium_value, age)
                
                # Get normal range
                normal_range = CALCIUM_CLINICAL_RANGES['Total_Calcium']['Normal']
                
                # Calculate deviation from normal range
                range_mid = (normal_range[0] + normal_range[1]) / 2
                deviation = ((calcium_value - range_mid) / range_mid) * 100
                ...
                analyzed_record = {
                    ...
                    'Age': age,
                    'Gender': gender,
                    'Calcium_Value': f"{calcium_value:.1f} mg/dL",
                    'Classification': classification,
                    'Normal_Range': f"{normal_range[0]:.1f}-{normal_range[1]:.1f} mg/dL",
                ...
                if row['Gold_Standard'] and str(row['Gold_Standard']).strip() != '':
                    try:
                        ...
                        accuracy_error = abs(calcium_value - gold_val)
    ...
    def display_results(self, analyzed_df):
        """Display calcium analysis results"""
        if analyzed_df.empty:
            print("[INFO] No results to display")
            return
        
        print("\n" + "="*80)
        print("[REPORT] CALCIUM ANALYSIS RESULTS")
        print("="*80)
        ...
        classification_order = {
            'Normal': 1, 
            'Hypocalcemia_Mild': 2, 
            'Hypercalcemia_Mild': 3,
            'Hypocalcemia_Severe': 4, 
            'Hypercalcemia_Severe': 5
        }
        analyzed_df['sort_order'] = analyzed_df['Classification'].map(classification_order).fillna(3)
        ...
        for _, row in sorted_results.iterrows():
            ...
            print(f"   [CA] Calcium: {row['Calcium_Value']}")
        ...
        # Summary statistics
        print("\n" + "="*80)
        print("[SUMMARY] CALCIUM CLASSIFICATION SUMMARY")
        print("="*80)
        ...
        for classification in ['Normal', 'Hypocalcemia_Mild', 'Hypocalcemia_Severe', 'Hypercalcemia_Mild', 'Hypercalcemia_Severe']:
        ...
        for gender, group_data in gender_groups:
            ...
            print(f"   {gender}: {abnormal_count}/{total_count} with abnormal calcium ({percentage:.1f}%)")
        ...
        for group_name, group_data in age_groups.items():
            if not group_data.empty:
                ...
                total_count = len(group_data)
                percentage = (abnormal_count / total_count * 100) if total_count > 0 else 0
                print(f"   {group_name}: {abnormal_count}/{total_count} with abnormal calcium ({percentage:.1f}%)")
        
        # Critical alerts
        severe_cases = analyzed_df[analyzed_df['Classification'].isin(['Hypocalcemia_Severe', 'Hypercalcemia_Severe'])]
        if not severe_cases.empty:
            print(f"\n🚨 CRITICAL ALERT: {len(severe_cases)} patients with SEVERE calcium abnormalities require IMMEDIATE attention!")
            for _, case in severe_cases.iterrows():
                print(f"   🔴 Patient {case['Patient_ID']}: {case['Calcium_Value']} (Normal: {case['Normal_Range']})")
    ...
    def save_excel_with_colors(self, analyzed_data):
        ...
        try:
            excel_file = "calcium_analyzed_results.xlsx"
            ...
            if self.append_mode and os.path.exists(excel_file):
                ...
                try:
                    ...
                    if "Calcium Analysis" in wb.sheetnames:
                        ws = wb["Calcium Analysis"]
                        ...
                        if existing_data:
                            ...
                            existing_df = pd.DataFrame(existing_data, columns=[
                                'Patient_ID', 'Test_Name', 'Age', 'Gender', 'Calcium_Value', 
            ...
            ws = wb.create_sheet("Calcium Analysis")
            ...
            ws['A3'] = f'Total Patients: {total_patients} | Normal: {normal_count} | Abnormal Calcium: {abnormal_count}'
    ...
    def _create_summary_worksheet(self, ws, analyzed_data):
        ...
        try:
            # Title
            ws['A1'] = 'CALCIUM ANALYSIS SUMMARY REPORT'
            ...
            row = 10
            # Use actual calcium classifications that the processor produces
            ca_classifications = ['Normal', 'Hypocalcemia_Mild', 'Hypocalcemia_Severe', 'Hypercalcemia_Mild', 'Hypercalcemia_Severe']
            
            for classification in ca_classifications:
                count = classification_counts.get(classification, 0)
                percentage = (count / total * 100) if total > 0 else 0
            ...
            for gender, group_data in gender_groups:
                ...
                ws[f'A{row}'] = f'{gender}:'
                ws[f'A{row}'].font = Font(size=11, bold=True)
                ws[f'B{row}'] = f'{abnormal_count}/{total_count} with abnormal calcium ({percentage:.1f}%)'
                ws[f'B{row}'].font = Font(size=11)
                row += 1
            
            # Critical alerts
            severe_cases = analyzed_data[analyzed_data['Classification'].isin(['Hypocalcemia_Severe', 'Hypercalcemia_Severe'])]
            if not severe_cases.empty:
                row += 1
                ...
                ws[f'A{row}'] = f'🚨 URGENT: {len(severe_cases)} patients with SEVERE calcium abnormalities!'
                ...
                for _, case in severe_cases.iterrows():
                    ...
                    ws[f'B{row}'] = f'{case["Calcium_Value"]} (Normal: {case["Normal_Range"]})'
            ...
            logger.info("[SUMMARY] Calcium summary worksheet created successfully")
            
        except Exception as e:
            logger.error(f"[ERROR] Error creating calcium summary worksheet: {e}")
            ...
            if not analyzed_data.empty:
                ...
                if excel_success:
                    if not self.append_mode:
                        print("   [SUCCESS] ✨ Excel file with colored data and summary report created!")
                    print("   [INFO] 📊 Two worksheets: 'Calcium Analysis' (data) + 'Summary Report' (analysis)")
    ...
    def run_complete_analysis(self):
        """Run complete calcium analysis workflow"""
        print("\n" + "="*80)
        print("[SYSTEM] CALCIUM MEDICAL DATA PROCESSOR")
        print("="*80)
        ...
        try:
            ...
            print(f"[SUCCESS] Processed {len(raw_data)} calcium records")
            
            # Step 2: Analyze data
            print("\n[STEP 2] Analyzing calcium data...")
            analyzed_data = self.analyze_calcium_data(raw_data)
            ...
            print("\n" + "="*80)
            print("[COMPLETE] Calcium analysis completed successfully!")
            print("="*80)
...
def main():
    ...
    try:
        print("\n" + "="*80)
        print("[CALCIUM PROCESSOR] Smart Excel Analysis")
        print("="*80)
        print("[INFO] This processor generates ONE Excel file with:")
        print("[INFO] • 'Calcium Analysis' sheet - Color-coded patient data")
        ...
        processor = CalciumProcessor(append_mode=append_mode)
        ...
        if success:
            ...
            if os.path.exists(excel_file):
                ...
                print("        ├── 'Calcium Analysis' sheet: Color-coded patient data with filtering")
